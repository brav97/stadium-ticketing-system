...
function EVENTS_insert(){
	...
	$data['date'] = mysql_datetime($_REQUEST['date']);
	...
	$data['seats'] = makeSafe($_REQUEST['seats']);
		if($data['seats'] == empty_lookup_value){ $data['seats'] = ''; }
	...
	sql('insert into `EVENTS` set       `teams`=' . (($data['teams'] !== '' && $data['teams'] !== NULL) ? "'{$data['teams']}'" : 'NULL') . ', `date`=' . (($data['date'] !== '' && $data['date'] !== NULL) ? "'{$data['date']}'" : 'NULL') . ', `endtime`=' . (($data['endtime'] !== '' && $data['endtime'] !== NULL) ? "'{$data['endtime']}'" : 'NULL') . ', `seats`=' . (($data['seats'] !== '' && $data['seats'] !== NULL) ? "'{$data['seats']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
...
function EVENTS_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	// child table: BOOK
	$res = sql("select `id` from `EVENTS` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `BOOK` where `teams`='".addslashes($id[0])."'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		...
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "BOOK", $RetMsg);
		$RetMsg = str_replace("<Delete>", "<input type=\"button\" class=\"button\" value=\"".$Translation['yes']."\" onClick=\"window.location='EVENTS_view.php?SelectedID=".urlencode($selected_id)."&delete_x=1&confirmed=1';\">", $RetMsg);
		$RetMsg = str_replace("<Cancel>", "<input type=\"button\" class=\"button\" value=\"".$Translation['no']."\" onClick=\"window.location='EVENTS_view.php?SelectedID=".urlencode($selected_id)."';\">", $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("select `id` from `EVENTS` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `RESERVE` where `Event`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "RESERVE", $RetMsg);
		return $RetMsg;
	}elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["confirm delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function EVENTS_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `EVENTS` set       `teams`=' . (($data['teams'] !== '' && $data['teams'] !== NULL) ? "'{$data['teams']}'" : 'NULL') . ', `date`=' . (($data['date'] !== '' && $data['date'] !== NULL) ? "'{$data['date']}'" : 'NULL') . ', `endtime`=' . (($data['endtime'] !== '' && $data['endtime'] !== NULL) ? "'{$data['endtime']}'" : 'NULL') . ', `seats`=' . (($data['seats'] !== '' && $data['seats'] !== NULL) ? "'{$data['seats']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function EVENTS_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)){
		...
		$jsReadOnly .= "\tjQuery('#seats').replaceWith('<div class=\"form-control-static\" id=\"seats\">' + (jQuery('#seats').val() || '') + '</div>');\n";
