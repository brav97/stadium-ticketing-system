	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'EVENTS' => array('Events', 'Add/edit/delet events  from this tab', 'resources/table_icons/calendar_edit.png', 'None'),
			'BOOK' => array('BOOK', 'Book a ticket From this tab', 'resources/table_icons/book_edit.png', 'None'),
			'RESERVE' => array('RESERVE TICKETS', 'Adding of tickets manually ', 'resources/table_icons/note_add.png', 'None'),
			'FANS' => array('FANS', 'Add your details here', 'resources/table_icons/teamwork.png', 'None'),
			'payments' => array('Payment channels', 'add your payment channels here', 'resources/table_icons/money.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'EVENTS' => "`EVENTS`.`id` as 'id', `EVENTS`.`teams` as 'teams', if(`EVENTS`.`date`,date_format(`EVENTS`.`date`,'%m/%d/%Y %h:%i %p'),'') as 'date', TIME_FORMAT(`EVENTS`.`endtime`, '%r') as 'endtime', `EVENTS`.`seats` as 'seats'",
			'BOOK' => "`BOOK`.`ID` as 'ID', IF(    CHAR_LENGTH(`EVENTS1`.`teams`) || CHAR_LENGTH(if(`EVENTS1`.`date`,date_format(`EVENTS1`.`date`,'%m/%d/%Y %h:%i %p'),'')), CONCAT_WS('',   `EVENTS1`.`teams`, '>>', if(`EVENTS1`.`date`,date_format(`EVENTS1`.`date`,'%m/%d/%Y %h:%i %p'),'')), '') as 'teams', IF(    CHAR_LENGTH(`FANS1`.`id_no`), CONCAT_WS('',   `FANS1`.`id_no`), '') as 'id_no', IF(    CHAR_LENGTH(`FANS1`.`phone`), CONCAT_WS('',   `FANS1`.`phone`), '') as 'phone', IF(    CHAR_LENGTH(`FANS1`.`Name`), CONCAT_WS('',   `FANS1`.`Name`), '') as 'name', IF(    CHAR_LENGTH(`payments1`.`channels`), CONCAT_WS('',   `payments1`.`channels`), '') as 'mode', IF(    CHAR_LENGTH(`payments1`.`mpesa`), CONCAT_WS('',   `payments1`.`mpesa`), '') as 'mpesa', IF(    CHAR_LENGTH(`payments1`.`paypal`), CONCAT_WS('',   `payments1`.`paypal`), '') as 'paypal', IF(    CHAR_LENGTH(`payments1`.`bank`), CONCAT_WS('',   `payments1`.`bank`), '') as 'bank', `BOOK`.`select` as 'select', `BOOK`.`enters` as 'enters', if(`BOOK`.`book_date`,date_format(`BOOK`.`book_date`,'%m/%d/%Y %h:%i %p'),'') as 'book_date'",
			'RESERVE' => "`RESERVE`.`id` as 'id', IF(    CHAR_LENGTH(`EVENTS1`.`teams`) || CHAR_LENGTH(if(`EVENTS1`.`date`,date_format(`EVENTS1`.`date`,'%m/%d/%Y %h:%i %p'),'')), CONCAT_WS('',   `EVENTS1`.`teams`, '>>', if(`EVENTS1`.`date`,date_format(`EVENTS1`.`date`,'%m/%d/%Y %h:%i %p'),'')), '') as 'Event', `RESERVE`.`assigned_to` as 'assigned_to', TIME_FORMAT(`RESERVE`.`time`, '%r') as 'time', `RESERVE`.`status` as 'status', `RESERVE`.`Amount_paid` as 'Amount_paid', `RESERVE`.`mode` as 'mode'",
			'FANS' => "`FANS`.`ID` as 'ID', `FANS`.`Name` as 'Name', `FANS`.`phone` as 'phone', `FANS`.`id_no` as 'id_no'",
			'payments' => "`payments`.`id` as 'id', `payments`.`channels` as 'channels', `payments`.`mpesa` as 'mpesa', `payments`.`paypal` as 'paypal', `payments`.`bank` as 'bank'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false) {
		$sql_from = array(   
			'EVENTS' => "`EVENTS` ",
			'BOOK' => "`BOOK` LEFT JOIN `EVENTS` as EVENTS1 ON `EVENTS1`.`id`=`BOOK`.`teams` LEFT JOIN `FANS` as FANS1 ON `FANS1`.`ID`=`BOOK`.`id_no` LEFT JOIN `payments` as payments1 ON `payments1`.`id`=`BOOK`.`mode` ",
			'RESERVE' => "`RESERVE` LEFT JOIN `EVENTS` as EVENTS1 ON `EVENTS1`.`id`=`RESERVE`.`Event` ",
			'FANS' => "`FANS` ",
			'payments' => "`payments` "
		);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> footticket</a>
			</div>
