		...
		$pcConfig = array(
			...
			'BOOK' => array(   
				'teams' => array(   
					...
					'tab-label' => 'BOOK <span class="hidden child-label-BOOK child-field-caption">(Event)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/book_edit.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Event', 2 => 'Id no', 3 => 'your Phone number', 4 => 'Name', 5 => 'Mode of payment', 6 => 'Mpesa Till number', 7 => 'Paypal pay channel', 8 => 'Bank wire', 9 => 'Select mode', 10 => 'Enter confirmation code', 11 => 'Booking date'),
					'display-field-names' => array(1 => 'teams', 2 => 'id_no', 3 => 'phone', 4 => 'name', 5 => 'mode', 6 => 'mpesa', 7 => 'paypal', 8 => 'bank', 9 => 'select', 10 => 'enters', 11 => 'book_date'),
					'sortable-fields' => array(0 => '`BOOK`.`ID`', 1 => 2, 2 => '`FANS1`.`id_no`', 3 => '`FANS1`.`phone`', 4 => '`FANS1`.`Name`', 5 => '`payments1`.`channels`', 6 => '`payments1`.`mpesa`', 7 => '`payments1`.`paypal`', 8 => '`payments1`.`bank`', 9 => 10, 10 => 11, 11 => '`BOOK`.`book_date`'),
					'records-per-page' => 10,
					...
					'template-printable' => 'children-BOOK-printable',
					'query' => "SELECT `BOOK`.`ID` as 'ID', IF(    CHAR_LENGTH(`EVENTS1`.`teams`) || CHAR_LENGTH(if(`EVENTS1`.`date`,date_format(`EVENTS1`.`date`,'%m/%d/%Y %h:%i %p'),'')), CONCAT_WS('',   `EVENTS1`.`teams`, '>>', if(`EVENTS1`.`date`,date_format(`EVENTS1`.`date`,'%m/%d/%Y %h:%i %p'),'')), '') as 'teams', IF(    CHAR_LENGTH(`FANS1`.`id_no`), CONCAT_WS('',   `FANS1`.`id_no`), '') as 'id_no', IF(    CHAR_LENGTH(`FANS1`.`phone`), CONCAT_WS('',   `FANS1`.`phone`), '') as 'phone', IF(    CHAR_LENGTH(`FANS1`.`Name`), CONCAT_WS('',   `FANS1`.`Name`), '') as 'name', IF(    CHAR_LENGTH(`payments1`.`channels`), CONCAT_WS('',   `payments1`.`channels`), '') as 'mode', IF(    CHAR_LENGTH(`payments1`.`mpesa`), CONCAT_WS('',   `payments1`.`mpesa`), '') as 'mpesa', IF(    CHAR_LENGTH(`payments1`.`paypal`), CONCAT_WS('',   `payments1`.`paypal`), '') as 'paypal', IF(    CHAR_LENGTH(`payments1`.`bank`), CONCAT_WS('',   `payments1`.`bank`), '') as 'bank', `BOOK`.`select` as 'select', `BOOK`.`enters` as 'enters', if(`BOOK`.`book_date`,date_format(`BOOK`.`book_date`,'%m/%d/%Y %h:%i %p'),'') as 'book_date' FROM `BOOK` LEFT JOIN `EVENTS` as EVENTS1 ON `EVENTS1`.`id`=`BOOK`.`teams` LEFT JOIN `FANS` as FANS1 ON `FANS1`.`ID`=`BOOK`.`id_no` LEFT JOIN `payments` as payments1 ON `payments1`.`id`=`BOOK`.`mode` "
				),
				'id_no' => array(   
					'parent-table' => 'FANS',
					'parent-primary-key' => 'ID',
					'child-primary-key' => 'ID',
					'child-primary-key-index' => 0,
					'tab-label' => 'BOOK <span class="hidden child-label-BOOK child-field-caption">(Id no)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/book_edit.png',
					'display-refresh' => true,
					'display-add-new' => true,
				...
				'mode' => array(   
					...
					'child-primary-key' => 'ID',
					'child-primary-key-index' => 0,
					'tab-label' => 'BOOK <span class="hidden child-label-BOOK child-field-caption">(Mode of payment)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/book_edit.png',
					'display-refresh' => true,
					'display-add-new' => true,
			...
			'RESERVE' => array(   
				'Event' => array(   
					...
					'child-primary-key' => 'id',
					'child-primary-key-index' => 0,
					'tab-label' => 'RESERVE A ATICKET <span class="hidden child-label-RESERVE child-field-caption">(Event)</span>',
					'auto-close' => true,
					'table-icon' => 'resources/table_icons/note_add.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Event', 2 => 'Assigned to', 3 => 'Time', 4 => 'Status', 5 => 'Amount paid(ksh)', 6 => 'Mode of payment'),
					'display-field-names' => array(1 => 'Event', 2 => 'assigned_to', 3 => 'time', 4 => 'status', 5 => 'Amount_paid', 6 => 'mode'),
					'sortable-fields' => array(0 => '`RESERVE`.`id`', 1 => 2, 2 => 3, 3 => '`RESERVE`.`time`', 4 => 5, 5 => 6, 6 => 7),
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-RESERVE',
					'template-printable' => 'children-RESERVE-printable',
					'query' => "SELECT `RESERVE`.`id` as 'id', IF(    CHAR_LENGTH(`EVENTS1`.`teams`) || CHAR_LENGTH(if(`EVENTS1`.`date`,date_format(`EVENTS1`.`date`,'%m/%d/%Y %h:%i %p'),'')), CONCAT_WS('',   `EVENTS1`.`teams`, '>>', if(`EVENTS1`.`date`,date_format(`EVENTS1`.`date`,'%m/%d/%Y %h:%i %p'),'')), '') as 'Event', `RESERVE`.`assigned_to` as 'assigned_to', TIME_FORMAT(`RESERVE`.`time`, '%r') as 'time', `RESERVE`.`status` as 'status', `RESERVE`.`Amount_paid` as 'Amount_paid', `RESERVE`.`mode` as 'mode' FROM `RESERVE` LEFT JOIN `EVENTS` as EVENTS1 ON `EVENTS1`.`id`=`RESERVE`.`Event` "
				)
			),
			'FANS' => array(   
			),
